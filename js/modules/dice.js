        ...
        <section class="dice-panel panel">
            ...
            <div class="arcane-table">
                <div class="attribute-area">
                    ...
                    <input class="alviryan-input" id="attribute-input" type="number" min="0" max="20" value="10">
                    <span>Dado Correspondente</span>
                    <h2 id="current-dice">d20</h2>
                </div>
            ...
            <button class="alviryan-button" id="roll-button">ROLAR DADO</button>
        </section>

        <aside class="side-panel">
            <div class="history-panel panel">
                <h2>Histórico</h2>
                <div id="history-list">Nenhuma rolagem.</div>
            </div>
...
function updateDice(){
    const attribute=Math.max(0, Math.min(20, Number(document.getElementById("attribute-input").value)));
    document.getElementById("attribute-input").value=attribute;
    document.getElementById("current-dice").textContent="d"+diceMap[attribute];
}
...
function rollDice(){
    ...
    // 1. RESET DE EMERGÊNCIA: Remove bloqueios anteriores e ativa a rotação
    ...
    function finishRoll(){
        ...
        // 2. DESLIGA A CLASSE DE ROLAGEM
        circle.classList.remove("rolling");
        
        // 3. FORÇA BRUTA: Desliga qualquer animação CSS ativa que cause rotação
        circle.style.animation = "none";
        resultElement.style.animation = "none";

        // 4. ESTABILIZAÇÃO SOBRE-HUMANA: Força o número a ficar 100% reto no centro com prioridade máxima
        resultElement.style.setProperty("transform", "rotate(0deg)", "important");
